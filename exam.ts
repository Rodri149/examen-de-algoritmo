...
console.log("Ejercicio 1: marcas de productos");
const contar =marcas.reduce((cuenta) => cuenta + 1, 0);
console.log(contar);

const ordenar = marcas.sort((a, b) => a.marca.localeCompare(b.marca));
console.log(ordenar);

console.log("Ejercicio 2: productos con mayor existencia");

const ProductoConMayorExiste = inventory.filter((product) => product.quantity ===30);
console.log(ProductoConMayorExiste);

const ProductosConMayorExistencia = ProductoConMayorExiste.map((product) => product.quantity);
console.log(ProductosConMayorExistencia);

console.log("Ejercicio 3: heroes por ciudad");

const cityContar = heroes.reduce((acc, hero) => {
...
for (const city in cityContar) {
    if (cityContar[city] > maxheroes) {
        cityMasHeroes = city;
        maxheroes = cityContar[city];
    }
}

console.log(cityContar);       
console.log("la cidudad con mayor cantidad es :" ,  cityMasHeroes);  



console.log("Ejercicio 4");

const sumarEdad =people.reduce((total, person) => total + person.edad, 0);
console.log(sumarEdad);

const promedioEdad = sumarEdad / people.length;
